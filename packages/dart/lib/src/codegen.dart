...
String _dartFieldType(LuxoField f) {
  if (f.isList) return 'List<${_luxoTypeToDart(f.type)}>';
  return _luxoTypeToDart(f.type);
}
...
String _jsonCast(LuxoField f) {
  ...
  if (f.isList && _isScalar(f.type)) {
    ...
    final cast = elem == 'double'
        ? "(json['$n'] as List).map((e) => (e as num).toDouble()).toList()"
        : "(json['$n'] as List).cast<$elem>()";
  ...
  switch (f.type) {
    ...
    default:
      ...
      if (f.isList) {
        final cast = "(json['$n'] as List).map((e) => ${f.type}.fromJson(e as Map<String, dynamic>)).toList()";
        return q ? "json['$n'] != null ? $cast : null" : cast;
      }
...
String _binaryRead(LuxoField f) {
  ...
  if (f.isList && _isScalar(f.type)) {
    switch (f.type) {
      ...
      default: return 'dec.readStringArray()';
    }
  }
  switch (f.type) {
  ...
  for (final api in schema.apis.values) {
    ...
    if (api.params.isNotEmpty) {
      ...
      for (final p in api.params) {
        if (p.isList) {
          b.writeln("    ParamSchema(${p.id}, '${p.name}', '${p.type}', true),");
        } else {
