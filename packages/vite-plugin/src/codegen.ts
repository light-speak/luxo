...
function genTypes(schema: LuxoSchema): string {
  ...
  out += "import { Decoder, ColumnarDecoder, decodeScalarArray } from '@luxojs/client'\n\n"
  ...
  for (const model of Object.values(schema.models)) {
    ...
    out += genColumnarDecode(model, schema, false)

    // Paginated columnar decoder
    out += genPaginatedDecode(model, schema, false)
  }
...
function columnTSType(field: LuxoField, schema: LuxoSchema): string {
  if (isNestedField(field, schema) || field.isList || field.list || field.type === 'Bytes') return 'Uint8Array[]'
...
function genColumnRead(field: LuxoField, schema: LuxoSchema): string {
  if (isNestedField(field, schema) || field.isList || field.list || field.type === 'Bytes') {
    return 'r.readColumnBytes()'
  }
  const n = field.nullable
  switch (field.type) {
...
function columnValue(field: LuxoField, schema: LuxoSchema): string {
  ...
  if (isNestedField(field, schema)) {
    ...
    if (field.nullable) {
      return `${column} && ${column}[i].length > 0 ? ${decoded} : null`
    }
...
function genSchema(schema: LuxoSchema): string {
  ...
  for (const api of Object.values(schema.apis)) {
    ...
    if (returnFields && returnFields.length > 0) {
      out += `, fields: { ${returnFields.map(field => `'${field.name}': ${field.id}`).join(', ')} }`
    }
    if (api.params && api.params.length > 0) {
      out += ', params: [\n'
      for (const p of api.params) {
        out += `    { fieldID: ${p.id}, name: '${p.name}', type: '${p.type}'${p.isList ? ', isList: true' : ''} },\n`
      }
      out += '  ]'
    }
    out += ' },\n'
...
function genMethod(api: LuxoAPI, schema: LuxoSchema): string {
  ...
  if (api.params && api.params.length > 0) {
    ...
    const paramFields = api.params.map(p => {
      if ((isPaginated && paginationNames.has(p.name)) || p.nullable || p.hasDefault) {
    ...
    if (isPaginated) {
      return `  async ${api.name}(params?: { ${paramFields}; $select?: string; $filters?: Filter[]; $sorters?: Sorter[] }, options?: CallOptions): Promise<${retTS}> {\n` +
             call('params') + `  }\n\n`
    }

    return `  async ${api.name}(params: { ${paramFields} }, options?: CallOptions): Promise<${retTS}> {\n` +
           call('params') + `  }\n\n`
  }

  // No params — zero-arg methods
  if (api.name.startsWith('list') && api.paginated) {
    return `  async ${api.name}(params?: { page?: number; pageSize?: number; $select?: string; $filters?: Filter[]; $sorters?: Sorter[] }, options?: CallOptions): Promise<${retTS}> {\n` +
           call('params') + `  }\n\n`
  }

  if (api.name.startsWith('create') && api.returnType) {
    return `  async ${api.name}(input: Partial<${api.returnType}>, options?: CallOptions): Promise<${retTS}> {\n` +
           call('input') + `  }\n\n`
  }

  if (api.name.startsWith('update') && api.returnType) {
    return `  async ${api.name}(id: number, input: Partial<${api.returnType}>, options?: CallOptions): Promise<${retTS}> {\n` +
           call('{ id, ...input }') + `  }\n\n`
  }
...
function genStreamMethod(api: LuxoAPI, schema: LuxoSchema, retTS: string, decode: string): string {
  ...
  const fields = api.params?.map(param => {
    const optional = param.nullable || param.hasDefault ? '?' : ''
    return `${param.name}${optional}: ${resolveParamType(param, schema)}`
  }).join('; ') ?? ''
